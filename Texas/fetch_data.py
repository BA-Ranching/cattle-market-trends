...
# NOTE: Fort Worth Superior (2713) and LiveAg (3892) are video/internet auctions.
# Their USDA reports use a narrative format that does not produce structured
# per-bracket price rows in the MARS API, so they are excluded from data fetching.
# They remain in MARKET_REPORT_SLUGS in index.html for the quick-link buttons.
MARKET_REPORTS = {
...
def build_bracket_matrix(cattle, cls_filter):
    ...
    # Aggregate by week × fixed bucket
...
def build_bracket_by_market(cattle):
    ...
    Structure: { market_name: { "steers": matrix, "heifers": matrix }, "_all": {...} }
    """
...
def cattle_latest_week(cattle):
    ...
    report on different schedules (e.g. one market reports a week later than others).
    Instead, find the most recent week where at least 2 markets have data; fall
    back to the absolute latest week only if just one market exists.
    """
    if not cattle: return []
    # Group available weeks by number of distinct markets reporting
    from collections import defaultdict
    ...
    # Find most recent week with the most market coverage
    sorted_weeks = sorted(week_markets.keys(), reverse=True)
    # Prefer the latest week that has 2+ markets; otherwise take the absolute latest
    best_week = sorted_weeks[0]  # fallback
